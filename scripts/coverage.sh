...
# crates/oasts-wasm is excluded on the same grounds: its extern "C" glue cannot execute under a
# native cargo test, so scripts/wasm-gate.sh runs it inside a wasm runtime instead.
...
set -euo pipefail
cd "$(dirname "$0")/.."
exec cargo llvm-cov --workspace --fail-under-lines 100 --ignore-filename-regex 'main\.rs|oasts-napi|oasts-wasm|oasts-bench' "$@"
