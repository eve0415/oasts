...
- Done means every `scripts/*.sh` gate is green. `scripts/verify-ts.sh` needs `cargo build` first. Run `scripts/gate.sh` (rustfmt + clippy + oxfmt + oxlint) before handing work back.
