...
test("rejects Node versions below the type-stripping floor", async () => {
  ...
  await expectFailure(loadScriptConfig(path, "20.19.0"), "OASTS0013", /requires Node >= 24/);
  await expectFailure(loadScriptConfig(path, "not-a-version"), "OASTS0013", /requires Node >= 24/);
});
