...
Every operation gets a descriptor — a plain `{ queryKey, queryFn }` or `{ mutationKey, mutationFn }`
object you spread into whatever adapter you already use. No hooks, no peer dependency, no React
context to carry the transport: generated code imports nothing from TanStack, so the same descriptor
works in React, Solid, Svelte, a route loader, or a prefetch.
...
modules import one leaf, so bundling a single descriptor does not drag in the rest of the spec's key
data; import the composed `keys` object when you want the whole tree.
...
<Aside type="caution">
  These are **prefix** keys. A query that carried query, header or cookie input has those appended
  as a further key element, so `exact: true` against the entity entry matches only the unfiltered
  query — invalidate without it to catch every filtered variant of the same resource.
</Aside>

## What does not get a query descriptor

A read gets one only when **every** documented success response carries a buffered body. Two things
disqualify an operation, and each emits an `OASTS1511` warning naming it rather than a descriptor
...
- **A [streaming](/guides/streaming) response.** A stream is consumable once and cannot be handed to
  two cache readers.

Non-reads are unaffected: a 204 `DELETE` gets a mutation descriptor, because mutations may resolve
`undefined`.
