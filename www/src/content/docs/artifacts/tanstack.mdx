...
`/pets/mine` and `/pets/{petId}` with `petId = "mine"` share every ancestor — `["api", "pets"]`,
which is exactly what makes prefix invalidation work — and then diverge at the segment where
`"mine"` meets `{ petId: "mine" }`. A string never equals an object, so nothing below that point can
collide the way a template-string key can. An optional path parameter you leave `undefined`
contributes `[]`, which is a segment of its own rather than a hole.
