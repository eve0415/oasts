...
type AssertPatternOnly = Expect<Equal<PatternOnly, { [key: `x-${string}`]: string }>>;
...
// @ts-expect-error the patternProperties value is string, not number.
export const patternRejectsNumber: PatternOnly = { "x-name": 1 };
...
export function exhaustsTaggedEvent(event: TaggedEvent): "empty" | "count" {
  switch (event.kind) {
    case "empty": {
